...
#[stable(feature = "rust1", since = "1.0.0")]
pub struct Arc<T: ?Sized, A: Alloc = AbortAdapter<Heap>> {
...
impl<T, A: Alloc> Arc<T, A> {
    ...
    pub fn new_in(data: T, alloc: A) -> Result<Arc<T, A>, A::Err> {
        ...
        Ok(Arc { ptr: Shared::from(unique), phantom: PhantomData })
    }
...
impl<T: ?Sized, A: Alloc> Arc<T, A> {
    ...
    unsafe fn from_box_raw(box_unique: Unique<T>, alloc: A) -> Result<Self, A::Err> {
        ...
        Ok(Arc { ptr: Shared::new_unchecked(ptr), phantom: PhantomData })
    }
...
impl<T, A: Alloc> Weak<T, A> {
    ...
    pub fn new_in(mut alloc: A) -> Result<Weak<T, A>, A::Err> {
        unsafe {
            let ptr = alloc.alloc(Layout::new::<ArcInner<T, A>>())? as *mut _;
