...
/// more memory efficient and make better use of CPU cache.
#[stable(feature = "rust1", since = "1.0.0")]
pub struct LinkedList<T, A: Alloc + Clone = AbortAdapter<Heap>> {
    head: Option<Shared<Node<T, A>>>,
    tail: Option<Shared<Node<T, A>>>,
    len: usize,
    alloc: A,
...
struct Node<T, A: Alloc> {
    next: Option<Shared<Node<T, A>>>,
    prev: Option<Shared<Node<T, A>>>,
    element: T,
    marker: PhantomData<Box<Node<T, A>, A>>,
}
...
#[stable(feature = "rust1", since = "1.0.0")]
pub struct Iter<'a, T: 'a, A: 'a + Alloc = AbortAdapter<Heap> > {
    head: Option<Shared<Node<T, A>>>,
    tail: Option<Shared<Node<T, A>>>,
    len: usize,
...
/// [`LinkedList`]: struct.LinkedList.html
#[stable(feature = "rust1", since = "1.0.0")]
pub struct IterMut<'a, T: 'a, A: Alloc + Clone + 'a = AbortAdapter<Heap>> {
...
/// [`LinkedList`]: struct.LinkedList.html
#[stable(feature = "rust1", since = "1.0.0")]
pub struct IntoIter<T, A: Alloc + Clone = AbortAdapter<Heap>> {
    list: LinkedList<T, A>,
}
...
impl<T, A: Alloc + Clone> LinkedList<T, A> {
    ...
    #[inline]
    unsafe fn unlink_node(&mut self, mut node: Shared<Node<T, A>>) {
...
impl<T, A: Alloc + Clone> LinkedList<T, A> {
    ...
    #[unstable(feature = "collection_placement",
               ...
               issue = "30172")]
    pub fn front_place(&mut self) -> Result<FrontPlace<T, A>, A::Err> {
        let alloc = self.alloc.clone();
        Ok(FrontPlace {
            list: self,
            node: IntermediateBox::make_place_in(alloc)?,
        })
    }
    ...
    #[unstable(feature = "collection_placement",
               reason = "method name and placement protocol are subject to change",
               issue = "30172")]
    pub fn back_place(&mut self) -> Result<BackPlace<T, A>, A::Err> {
        let alloc = self.alloc.clone();
        Ok(BackPlace {
            list: self,
            node: IntermediateBox::make_place_in(alloc)?,
    ...
    pub fn insert_next_alloc(&mut self, element: T) -> Result<(), A::Err> {
        Ok(match self.head {
            ...
            Some(mut head) => unsafe {
                ...
                let node = Some(Shared::from(Box::into_unique(Box::new_in(Node {
                    next: Some(head),
                    prev: Some(prev),
                    element,
                    marker: PhantomData,
...
#[unstable(feature = "drain_filter", reason = "recently added", issue = "43244")]
pub struct DrainFilter<'a, T: 'a, F: 'a, A: 'a = AbortAdapter<Heap>>
    where A: Alloc + Clone, F: FnMut(&mut T) -> bool,
{
    list: &'a mut LinkedList<T, A>,
    it: Option<Shared<Node<T, A>>>,
    pred: F,
    idx: usize,
    old_len: usize,
}
...
unsafe fn finalize<T, A: Alloc>(node: IntermediateBox<Node<T, A>, A>) -> Box<Node<T, A>, A> {
...
#[unstable(feature = "collection_placement",
           ...
           issue = "30172")]
pub struct FrontPlace<'a, T: 'a, A: 'a + Alloc + Clone = AbortAdapter<Heap>> {
    list: &'a mut LinkedList<T, A>,
    node: IntermediateBox<Node<T, A>, A>,
}

#[unstable(feature = "collection_placement",
           reason = "struct name and placement protocol are subject to change",
           issue = "30172")]
impl<'a, T: 'a + fmt::Debug, A: 'a + Alloc + Clone> fmt::Debug for FrontPlace<'a, T, A> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
#[unstable(feature = "collection_placement",
           ...
           issue = "30172")]
impl<'a, T, A: Alloc + Clone> Placer<T> for FrontPlace<'a, T, A> {
    type Place = Self;
...
#[unstable(feature = "collection_placement",
           reason = "placement protocol is subject to change",
           issue = "30172")]
impl<'a, T, A: Alloc + Clone> Place<T> for FrontPlace<'a, T, A> {
...
#[unstable(feature = "collection_placement",
           reason = "placement protocol is subject to change",
           issue = "30172")]
impl<'a, T, A: Alloc + Clone> InPlace<T> for FrontPlace<'a, T, A> {
    type Owner = ();
...
#[unstable(feature = "collection_placement",
           reason = "struct name and placement protocol are subject to change",
           issue = "30172")]
pub struct BackPlace<'a, T: 'a, A: 'a + Alloc + Clone = AbortAdapter<Heap>> {
    list: &'a mut LinkedList<T, A>,
    node: IntermediateBox<Node<T, A>, A>,
}
...
#[unstable(feature = "collection_placement",
           reason = "struct name and placement protocol are subject to change",
           issue = "30172")]
impl<'a, T: 'a + fmt::Debug, A: Alloc + Clone> fmt::Debug for BackPlace<'a, T, A> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
#[unstable(feature = "collection_placement",
           reason = "placement protocol is subject to change",
           issue = "30172")]
impl<'a, T, A: Alloc + Clone> Placer<T> for BackPlace<'a, T, A> {
    type Place = Self;
...
#[unstable(feature = "collection_placement",
           reason = "placement protocol is subject to change",
           issue = "30172")]
impl<'a, T, A: Alloc + Clone> Place<T> for BackPlace<'a, T, A> {
    fn pointer(&mut self) -> *mut T {
        unsafe { &mut (*self.node.pointer()).element }
    }
}
...
#[unstable(feature = "collection_placement",
           reason = "placement protocol is subject to change",
           issue = "30172")]
impl<'a, T, A: Alloc + Clone> InPlace<T> for BackPlace<'a, T, A> {
    type Owner = ();
...
mod tests {
    ...
    type HeapNode<T> = Node<T, AbortAdapater<Heap>>;
    ...
    pub fn check_links<T>(list: &LinkedList<T, A>) {
        unsafe {
            let mut len = 0;
            let mut last_ptr: Option<&HeapNode<T>> = None;
            let mut node_ptr: &HeapNode<T>;
            match list.head {
                None => {
            ...
            loop {
                match (last_ptr, node_ptr.prev) {
                    ...
                    (Some(p), Some(pptr)) => {
                        assert_eq!(p as *const HeapNode<T>, pptr.as_ptr() as *const HeapNode<T>);
                    }
            ...
            assert_eq!(tail as *const HeapNode<T>, node_ptr as *const HeapNode<T>);
